...
- 单位换算：按分类进行数据大小、时间、速率、百分比、货币进制等换算。
...
- 内置单位换算工具（支持数据大小/时间/流量/带宽/百分比/人民币/货币进制）
- 单位换算支持“智能展示”结果（自动选择更易读单位）
- 单位换算输入支持“数值 + 单位”格式（例如 `28717740816392 byte`）
...
- 内含四个页面：`translator.html`（字幕翻译）、`json-tool.html`、`timestamp.html`、`unit-converter.html`
...
1. 打开 `http://localhost:3000/unit-converter.html`
2. 选择分类（如“数据大小”）
3. 输入数值（支持纯数字或“数字 + 单位”，例如 `28717740816392 byte`）
4. 选择源单位与目标单位（可用“交换单位”快速互换）
5. 查看两种结果：
   - 换算结果：按你选择的目标单位输出
   - 智能展示：自动选择更易读的单位输出
6. 需要时点击“复制结果”写入剪贴板
...
- 输入：`28717740816392 byte`
- 智能展示：`28.717740816392 Tb`

说明：

- “货币”分类当前仅做同币种单位换算（例如分/元），不做实时汇率换算。
